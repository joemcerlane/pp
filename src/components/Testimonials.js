      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {testimonials.map((item, idx) => (
          <div key={idx} className="bg-gray-50 px-8 py-10 rounded-md">
            <p className="font-normal text-gray-500 text-md mb-4">{item.text}</p>
            <h6 className="font-semibold text-gray-500 text-md">{item.author} <span className="font-medium text-gray-300 text-sm">- {item.position}</span></h6>
          </div>
        ))}
      </div>
    </div>
